...
const ProTicker = () => {
    ...
    return (
        <div className="w-full bg-slate-955 border-b border-slate-800 py-1.5 overflow-hidden sticky top-[57px] z-[40] backdrop-blur-sm">
...
export default function CryptoProClient({ locale, dict }: { locale: string, dict: any }) {
    ...
    return (
        <div className="min-h-screen bg-[#060913] text-slate-200 selection:bg-amber-950 selection:text-amber-200">
            ...
            <main className="max-w-[1440px] mx-auto grid grid-cols-1 lg:grid-cols-12 border-x border-slate-800 bg-[#060913] min-h-screen shadow-2xl">
                ...
                <div className="lg:col-span-8 p-6 md:p-12 lg:p-16 space-y-20 border-r border-slate-800/80">
                    ...
                    <section className="space-y-10 pt-16 border-t border-slate-800/85">
                        <div className="flex items-center justify-between">
                            <h2 className="text-[13px] font-black uppercase tracking-[0.4em] flex items-center gap-3 text-white leading-none">
                                <Bitcoin className="w-4 h-4 text-amber-400 animate-pulse" />
                                {dict.crypto_pro.terminal_title}
                            </h2>
                            <div className="text-[9px] font-black text-slate-500 tracking-widest">{dict.crypto_pro.onchain_synced}</div>
                        </div>
                    ...
                    <section className="space-y-10 pt-16 border-t border-slate-800/80">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="bg-slate-950/60 p-8 rounded-none space-y-4 border border-slate-800">
                                <div className="text-[11px] font-black text-indigo-400 uppercase tracking-widest">{dict.crypto_pro.market_inter_sync}</div>
                                <h4 className="text-lg font-black tracking-tight text-white font-sans uppercase">{dict.crypto_pro.btc_vs_us10y_title}</h4>
                                <p className="text-xs text-slate-400 font-medium leading-relaxed uppercase">
                                    {dict.crypto_pro.btc_vs_us10y_desc}
                                </p>
                            </div>
                            <div className="bg-slate-950/60 p-8 rounded-none space-y-4 border border-slate-800">
                                <div className="text-[11px] font-black text-emerald-400 uppercase tracking-widest">{dict.crypto_pro.onchain_pulse}</div>
                                <h4 className="text-lg font-black tracking-tight text-white font-sans uppercase">{dict.crypto_pro.whale_accumulation_title}</h4>
                                <p className="text-xs text-slate-400 font-medium leading-relaxed uppercase">
                    ...
                    <section className="space-y-12 pt-16 border-t border-slate-800/80">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            <div className="space-y-6">
                                ...
                                <div className="p-8 bg-slate-955 border border-slate-800 rounded-none space-y-6">
                                    <div className="flex flex-col gap-4">
                                        {[
                                            ...
                                            <div key={i} className="space-y-2">
                                                <div className="text-[11px] font-black text-slate-500 uppercase">{depth.range}</div>
                                                <div className="grid grid-cols-2 gap-1 bg-slate-900/60 p-2 border border-slate-800">
                                                    <div className="text-center">
                                                        <div className="text-[11px] font-black text-emerald-450 uppercase">{dict.crypto_pro.bid_side}</div>
                                                        <div className="text-xs font-black text-white">{depth.bid}</div>
                                                    </div>
                                                    <div className="text-center border-l border-slate-800">
                                                        <div className="text-[11px] font-black text-rose-450 uppercase">{dict.crypto_pro.ask_side}</div>
                                                        <div className="text-xs font-black text-white">{depth.ask}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                    <p className="text-[11px] font-bold text-slate-505 leading-relaxed uppercase">
                                ...
                                <div className="p-8 bg-slate-950/60 border border-slate-800 rounded-none space-y-6">
                                    <div className="grid grid-cols-2 gap-6">
                                        <div className="space-y-1">
                                            <div className="text-[11px] font-black text-slate-500 uppercase">{dict.crypto_pro.network_load}</div>
                                            <div className="text-xl font-black text-white tabular-nums uppercase">{dict.crypto_pro.network_stable}</div>
                                        </div>
                                        <div className="space-y-1">
                                            <div className="text-[11px] font-black text-slate-500 uppercase">{dict.crypto_pro.avg_gas_price}</div>
                                            <div className="text-xl font-black text-white tabular-nums uppercase">12 Gwei</div>
                                        </div>
                                    </div>
                                    <div className="h-20 flex items-center gap-1">
                                        {[40, 20, 60, 30, 80, 50, 90, 40, 70].map((v, i) => (
                                            <div key={i} className="flex-1 bg-amber-500/20 border border-amber-500/20 rounded-none" style={{ height: `${v}%` }} />
                                        ))}
                                    </div>
                    ...
                    <section className="space-y-10 pt-16 border-t border-slate-800/80">
                        ...
                        <div className="space-y-4">
                            {[
                                ...
                                <div key={i} className="flex items-center justify-between p-5 bg-slate-900/40 border border-slate-800 rounded-none relative overflow-hidden">
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div className="space-y-1">
                                            <div className="text-[12px] font-black text-white uppercase font-sans">{whale.amount}</div>
                                            <div className="text-[10px] font-bold text-slate-500 font-mono">{whale.address}</div>
                                        </div>
                                    </div>
                                    <div className="text-right space-y-1">
                                        <div className={`text-[11px] font-black uppercase tracking-widest ${whale.type === 'whale_type_inflow' ? 'text-rose-455' : whale.type === 'whale_type_outflow' ? 'text-emerald-450' : 'text-indigo-400'}`}>{dict.crypto_pro[whale.type]}</div>
                                        <div className="text-[11px] font-bold text-slate-500 uppercase">→ {whale.target.startsWith('whale_') ? dict.crypto_pro[whale.target] : whale.target}</div>
                                    </div>
                                </div>
                            ))}
                        </div>
...
const CryptoCard = ({ label, status, sync, levels, consistency, dict }: any) => {
    ...
    return (
        <div className="bg-slate-900/40 border border-slate-800 rounded-none p-6 space-y-6 hover:shadow-[0_0_20px_rgba(245,158,11,0.1)] hover:border-amber-500/80 transition-all cursor-crosshair group">
            <div className="flex items-center justify-between">
                ...
                <span className={`text-[9px] font-black px-1.5 py-0.5 rounded-none tracking-widest uppercase border ${
                    status === 'status_divergent' || status === 'status_in_progress' ? 'bg-amber-950/40 border-amber-800/60 text-amber-400' :
            ...
            <div className="grid grid-cols-4 gap-1">
                {['15M', '1H', '4H', '1D'].map((tf, i) => {
                    const dir = levels[i];
                    return (
                        <div key={tf} className="bg-slate-955 p-2 rounded-none border border-slate-850 flex flex-col items-center gap-1.5">
                            ...
                            {dir === false && <TrendingUp className="w-4 h-4 text-rose-455 rotate-180" />}
            ...
            <div className="space-y-1.5">
                ...
                <div className="w-full h-1 bg-slate-950 rounded-none overflow-hidden">
                    <div className="h-full bg-amber-500 shadow-[0_0_6px_rgba(245,158,11,0.6)] transition-all duration-1000" style={{ width: `${sync}%` }} />
                </div>
            </div>
            ...
            <div className="flex items-center justify-between border-t border-slate-800/65 pt-4">
                ...
                <button 
                    ...
                    className="text-[11px] font-black text-amber-450 uppercase tracking-widest hover:underline transition-all cursor-pointer"
                >
            ...
            >
                <div className="space-y-8">
                    <div className="flex items-center justify-between p-6 bg-slate-950/80 border border-slate-800">
                        ...
                        <div className="text-right space-y-1">
                            <div className="text-[11px] font-black text-slate-500 uppercase tracking-widest">{dict.crypto_pro.market_consistency}</div>
                            <div className="text-xl font-black text-amber-450">{dict.pro[consistency]}</div>
                        </div>
                    </div>

                    <div className="space-y-4">
                        <div className="text-[11px] font-black text-white uppercase tracking-[0.2em] border-b border-slate-800 pb-2">{dict.crypto_pro.onchain_liquidity_matrix}</div>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-4 bg-slate-900/60 border border-slate-800 space-y-2">
                                <div className="text-[10px] font-black text-slate-505 uppercase">{dict.pro.sync_rate}</div>
                                <div className="text-lg font-black text-white tabular-nums">{sync}%</div>
                                <div className="w-full h-1 bg-slate-950">
                                    <div className="h-full bg-amber-500" style={{ width: `${sync}%` }} />
                                </div>
                            </div>
                            <div className="p-4 bg-slate-900/60 border border-slate-800 space-y-2">
                                <div className="text-[10px] font-black text-slate-550 uppercase">{dict.crypto_pro.volatility}</div>
                                <div className="text-lg font-black text-white uppercase">{sync > 80 ? dict.crypto_pro.vola_stable : dict.crypto_pro.vola_active}</div>
                                <div className="flex gap-1">
                                    {[1, 2, 3, 4, 5].map(i => (
                                        <div key={i} className={`h-1 flex-1 ${i <= (sync > 80 ? 4 : 2) ? 'bg-emerald-500' : 'bg-slate-800'}`} />
                                    ))}
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="p-6 border border-slate-850 space-y-4">
                        <div className="text-[11px] font-black text-white uppercase tracking-widest">{dict.crypto_pro.ai_onchain_diagnosis}</div>
